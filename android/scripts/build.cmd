...
:opencv-cmake
("%CMAKE%" -G"%CMAKE_GENERATOR%" -DANDROID_ABI="%ANDROID_ABI%" -DCMAKE_TOOLCHAIN_FILE="%SOURCE_DIR%\android.toolchain.cmake" %ANDROID_TOOLCHAIN_CMAKE% -DCMAKE_MAKE_PROGRAM="%MAKE%" %* "%SOURCE_DIR%\.." -DBUILD_SHARED_LIBS:BOOL=ON -DBUILD_ANDROID_EXAMPLES:BOOL=OFF -DWITH_CUDA:BOOL=OFF -DBUILD_PERF_TESTS:BOOL=OFF -DBUILD_TIFF:BOOL=OFF -DWITH_TIFF:BOOL=OFF -DBUILD_DOCS:BOOL=OFF -DBUILD_TESTS:BOOL=OFF -DBUILD_WITH_DEBUG_INFO:BOOL=OFF -DBUILD_opencv_ts:BOOL=OFF -DWITH_OPENCL:BOOL=OFF -DBUILD_opencv_androidcamera:BOOL=OFF) && GOTO cmakefin
